...
var ReleaseTools = module.exports = function releaseTools(options) {
  ...
  this.updateChangelog = function(callback) {
    ...
    Step(
      function(){
        exec('git checkout History.md', this);
      },
      function(){
      ...
      function(err, tag) {
        ...
        if (tag !== '') {
          if (tag == getCurrentVersion()) {
            throw 'You have to first update your package.json with the release version';
          }
          interval = tag + '..HEAD ';
        }        
        ...
        exec('git log ' + interval + '--pretty=format:"  * %s"', function(err, stdout, stderr) {
          ...
          changelog += '\n' + stdout + '\n\n';
          try {
            if (fs.statSync(path.join(options.currentPath, 'History.md')).isDirectory()) {
  ...
  this.commitToGit = function(callback) {  
    ...
    Step(
      function() {
        exec('git add AUTHORS docs/* History.md', this);
      },
      function() {
