...
var ReleaseTools = module.exports = function releaseTools(options) {
  ...
  this.createSite = function(callback) {
    ...
    var html = fs.readFileSync(path.join(
      options.currentPath, 'site', '_header.html'), 'utf8'
    );
    html += markdown.toHtmlSync(
      fs.readFileSync(path.join(options.currentPath, "README.md"), 'utf8')
    );
    html += markdown.toHtmlSync(
      fs.readFileSync(path.join(options.currentPath, "History.md"), 'utf8')
    );
    html += fs.readFileSync(path.join(options.currentPath, 'site', '_footer.html'));
  ...
  this.createAuthorsFile = function(callback) {
    exec('git shortlog -nse | cut -c8- >| AUTHORS', function(err, stdout, stderr) {
      if (err) {
  ...
  this.updateChangelog = function(version, callback) {
    ...
    exec('git tag | tail -1', function(err, stdout, stderr) {
      if (err) {
        callback("Error while getting the last git tag: " + err);
        return;
      }
  ...
  this.createGitTag = function(version, callback) {  
    Step(
      function() {
